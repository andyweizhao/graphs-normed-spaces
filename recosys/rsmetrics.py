...
class RankingBuilder:
    ...
    def rank(self, model, eval_batch, n_random_items=100):
        ...
        random_scores = scores.reshape(-1, n_random_items).cpu().numpy()

        target_scores = torch.unsqueeze(model(eval_batch), 1).cpu().numpy()
    ...
    def build_input_tensor(self, user_id, random_ids):
        ...
        random_ids = torch.unsqueeze(torch.LongTensor(random_ids), 1).to(user_ids.device)   # n x 1
